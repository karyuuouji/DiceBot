...
public class DiceWebAPI {
	...
	private static DiceResponse Request(Map<String, String> formData,
		...
		try {
			...
			//EasySSLSocketFactory ea = new EasySSLSocketFactory();
			//ea.createSocket();
			//DiceData diceData = new DiceData(form, response);
	...
	/*private class DiceConnect extends AsyncTask<String, Void, DiceResponse> {
		...
		public DiceConnect(DiceData diceData){
			super(diceData);
			response = diceData.getResponse();
			formData = diceData.getForm();
		}
		
		//@Override
		protected void onPreExecute(Map<String,String> formData){

		}
		
        @Override
        protected DiceResponse doInBackground(String... string) {
        ...
        protected DiceResponse onPostExecute() {
